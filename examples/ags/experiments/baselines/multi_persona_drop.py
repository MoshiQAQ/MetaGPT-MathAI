...
class DebateAgent(Operator):
    ...
    async def __call__(self, inputs: str, context: List[str] = None, mode: str = None):
        role_prompt = f"You are a {self.role}. Based on your professional knowledge and thinking style,"
        if context is None:
            prompt = role_prompt + DEBATE_INITIAL_PROMPT.format(inputs=inputs)
        else:
            prompt = role_prompt + DEBATE_PROMPT.format(inputs=inputs) + "\n".join(context)
...
class FinalDecisionAgent(Operator):
    ...
    async def __call__(self, inputs: str, all_thinking: List[str], all_answers: List[str], mode: str = None):
    ...
    async def __call__(self, inputs):
        ...
        for r in range(max_round):
            for i, agent in enumerate(self.debate_agents):
                if r == 0:
                    result = await agent(inputs, mode="context_fill")
                else:
                    ...
                    result = await agent(inputs, context, mode="context_fill")
...
if __name__ == "__main__":
    async def main():
        ...
        graph = MultiPersonaGraph(name="multi-persona", llm_config=llm_config, dataset="DROP")
